package org.mqjd.spark.scala.spark11
...
object FinalChargeTimeDF extends SparkDFBase {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val startTransactionRequestsDF = input
      .transform(filterStartTransactionRequests)
      .transform(convertStartTransactionRequestBody)
    val startTransactionResponsesDF = input
      .transform(filterStartTransactionResponses)
      .transform(convertStartTransactionResponseBody)
    val startTransactionsDF =
      joinStartTransactions(startTransactionRequestsDF, startTransactionResponsesDF)
    val stopTransactionRequestsDF =
      input.transform(filterStopTransactionRequests).transform(convertStopTransactionRequestBody)
    val calculateTotalDF =
      joinStartStopTransactions(startTransactionsDF, stopTransactionRequestsDF)
        .transform(calculateTotalTimeHours)
        .transform(calculateTotalEnergy)
    val meterValuesDF = input
      .transform(filterMeterValues)
      .transform(convertMeterValueBody)
      .transform(reshapeMeterValues)
      .transform(calculateTotalParkingTimeHours)
  ...
  private def convertStartTransactionResponseBody(input: DataFrame): DataFrame = {
    ...
    val bodySchema = StructType(
      Seq(
        StructField("transaction_id", IntegerType, nullable = true),
        StructField("id_tag_info", idTagSchema, nullable = true)
      )
    )
    input.withColumn("new_body", from_json(input.col("body"), bodySchema))
  }
  ...
  private def joinStartTransactions(requestDF: DataFrame, responseDF: DataFrame): DataFrame = {
    requestDF
      .as("request_df")
      .join(responseDF.as("response_df"), Seq("message_id"))
      .select(
  ...
  private def calculateTotalTimeHours(inputDF: DataFrame): DataFrame = {
    inputDF
      .withColumn(
        "total_time",
        col("stop_timestamp").cast("long") / secondsInOneHour - col("start_timestamp").cast("long") / secondsInOneHour
      )
  ...
  private def convertMeterValueBody(input: DataFrame): DataFrame = {
    val sampledValueSchema = ArrayType(
      StructType(
        Seq(
    ...
    val meterValueSchema = ArrayType(
      StructType(
        Seq(
          StructField("timestamp", StringType, nullable = true),
          StructField("sampled_value", sampledValueSchema, nullable = true)
        )
      )
    )
    val bodySchema = StructType(
      Seq(
        StructField("connector_id", IntegerType, nullable = true),
        StructField("transaction_id", IntegerType, nullable = true),
        StructField("meter_value", meterValueSchema, nullable = true)
      )
    )
    input.withColumn("new_body", from_json(input.col("body"), bodySchema))
  }
  ...
  private def reshapeMeterValues(input: DataFrame): DataFrame = {
    input
      .withColumn("meter_value", explode(col("new_body.meter_value")))
  ...
  private def calculateTotalParkingTimeHours(inputDF: DataFrame): DataFrame = {
    ...
    inputDF
      .withColumn("value", when(col("value") > 0, 1).otherwise(0))
