...
public class ParallelismAdjustmentTest extends FlinkJobTest {
    ...
    public void given_correct_input_and_output_when_execute_then_success() throws Exception {
        ...
        Path jobCheckpointsPath = new Path(
            environment.getJobConfig().get(CheckpointingOptions.CHECKPOINTS_DIRECTORY));
        FileSystem fileSystem = jobCheckpointsPath.getFileSystem();
        FileStatus latestCheckpoint = Arrays.stream(
                fileSystem.listStatus(new Path(jobCheckpointsPath, jobID.toString())))
            .filter(v -> v.getPath().getName().contains("chk")).findAny()
            .orElseThrow(() -> new RuntimeException("Checkpoint not found"));
        String executionSavepointPath = latestCheckpoint.getPath().getPath();
        String[] recoveryParams = {"-D",
            STR."job-config.execution.savepoint.path=file://\{executionSavepointPath}", "-D",
            "job-config.parallelism.default=8"};
