...
/**
 * Create and populate a unique instance of a MySQL database for each run of JUnit test. A user of class needs to
 * provide a logical name for Debezium and database name. It is expected that there is a init file in
 * <code>src/test/resources/ddl/&lt;database_name&gt;.sql</code>. The database name is enriched with a unique suffix
 * that guarantees complete isolation between runs <code>
...
public class UniqueDatabase {
    ...
    private UniqueDatabase(MySqlContainer container, String databaseName, final String identifier, String username,
        String password) {
    ...
    public void createAndInitialize(String chapter, String section) {
        ...
        try {
            try (Connection connection = DriverManager.getConnection(container.getJdbcUrl(), username, password);
                ...
                final List<String> statements = Arrays.stream(Stream
                    .concat(Arrays.stream(CREATE_DATABASE_DDL),
